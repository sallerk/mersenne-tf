    ...
    __global uint        *phase_total)
{
    ...
    uint base = w << 5;
    uint v = ~bits[w];                       // clear bit = survivor
    ...
    uint pc = popcount(v);
    atomic_add(phase_total, pc);
    uint at = atomic_add(count, pc);
    while (v) {
        uint lsb = v & (0u - v);
        idx[at++] = base + (31u - clz(lsb));  // ctz is OpenCL 2.0; this is 1.2
        v &= v - 1u;
    }
}
)CLC";
