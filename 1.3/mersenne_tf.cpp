...
typedef cl_int (CLAPI *P_clGetEventProfilingInfo)(cl_event, cl_uint, size_t, void*, size_t*);
...
static struct {
    ...
    P_clGetEventProfilingInfo GetEventProfilingInfo;
...
// Diagnostic (--profile): per-kernel device time, so the sieve's cost can be
// attributed to a kernel instead of inferred from whole-run differences.
//
// Each launch is waited on and its timestamps read immediately, which serialises
// the pipeline -- the WALL TIME OF A PROFILED RUN IS MEANINGLESS and is not
// reported.  The per-kernel figures are the device's own start/end stamps for
// the kernel itself, so they stay valid: what serialising removes is overlap
// between launches, not the duration of any one of them.
static bool g_profile = false;

// --no-fuse: run the split sieve_compact + TF pipeline even where the fused
// kernel exists.  Kept so the two can be compared on any machine.
static bool g_fuse = true;

// Work-group size of the fused kernel.  It declares reqd_work_group_size and
// sizes its LDS from this, so the host must launch it with exactly this and
...
enum { KP_OFFSETS, KP_SMALL, KP_TIER, KP_LARGE, KP_HUGE, KP_COMPACT, KP_TF, KP_N };
static const char* const KP_NAME[KP_N] = {
    "sieve_offsets", "sieve_mark_small", "sieve_mark_tier",
    "sieve_mark_large", "sieve_mark_huge", "sieve_compact", "trial factoring"
};
static double   g_kp_ns[KP_N] = { 0 };
static uint64_t g_kp_n [KP_N] = { 0 };

// Wait for one launch and add its device time to the bucket.  Consumes the event.
static void kp_record(int slot, cl_event e)
{
    if (!e) return;
    if (CL.GetEventProfilingInfo) {
        cl_ulong t0 = 0, t1 = 0;
        CL.WaitForEvents(1, &e);
        if (CL.GetEventProfilingInfo(e, CL_PROFILING_COMMAND_START, sizeof(t0), &t0, nullptr) == CL_SUCCESS &&
            CL.GetEventProfilingInfo(e, CL_PROFILING_COMMAND_END,   sizeof(t1), &t1, nullptr) == CL_SUCCESS &&
            t1 >= t0) {
            g_kp_ns[slot] += (double)(t1 - t0);
            g_kp_n [slot] += 1;
        }
    }
    CL.ReleaseEvent(e);
...
#define GETPROC(name, field, required)                                        \
    ...
    GETPROC(GetEventProfilingInfo, GetEventProfilingInfo, false)
...
// the same GPU and nothing is hidden.
//
// The device bracket USED to be 120000/250000, because the device sieve got
// rapidly more expensive with depth: its cost scaled with the number of primes
// rather than with the number of strikes, so past ~120000 each extra prime cost
// more than the candidates it removed.  sieve_mark_huge removed that scaling and
// the curve inverted.  Measured over a whole bit level, p=86000009, 2^66..2^67,
// RTX 3070 -- wall seconds, so lower is better at every point:
//
//   sieve_primes   120k   250k   500k   1.04M   2M     4M     8M     16M    32M
//   before         9.48   9.82   11.88  16.78   --     --     --     --     --
//   after          8.41   8.19   8.05    7.90   7.76   7.70   7.60   7.70   8.05
//
// Deeper is now better until the prime table itself starts to cost, and the
// floor is flat from about 2M to 16M.  4M sits inside that with half the table
// of the measured minimum, which matters on a smaller card.
//
...
static uint32_t resolve_sieve_limit(const Config& cfg, uint64_t p)
{
    ...
    if (cfg.sieve_on_gpu) return (pbits <= 32) ? 2000000u : 4000000u;
...
static bool gpu_init(Gpu& g, int want_platform, int want_device, std::string& err)
{
    ...
    CL.GetDeviceInfo(g.device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(g.lds), &g.lds, nullptr);
    ...
    // --profile needs the compute queue to carry timestamps; the transfer queue
    // below does not, so it is left alone.
    const cl_ulong qprops[3] = { CL_QUEUE_PROPERTIES, CL_QUEUE_PROFILING_ENABLE, 0 };
    if (CL.CreateCommandQueueWithProperties) {
        g.queue = CL.CreateCommandQueueWithProperties(g.ctx, g.device,
                                                      g_profile ? qprops : nullptr, &st);
    } else {
        g.queue = CL.CreateCommandQueue(g.ctx, g.device,
                                        g_profile ? CL_QUEUE_PROFILING_ENABLE : 0, &st);
    }
    ...
    char bopts[64];
    snprintf(bopts, sizeof(bopts), "-cl-std=CL1.2 -DTFGS_WG=%u", (unsigned)TFGS_WG);
    st = CL.BuildProgram(g.program, 1, &g.device, bopts, nullptr, nullptr);
    if (st != CL_SUCCESS) {
        size_t n = 0;
...
static void gpu_free(Gpu& g)
{
    ...
    if (g.k_huge)    CL.ReleaseKernel(g.k_huge);
                      ...
                      std::string& err)
{
    ...
    uint32_t huge_first  = 0;                  // ... and above the tile -> huge kernel
    ...
    if (cfg.sieve_on_gpu && !sp.empty()) {
        ...
        // A prime much larger than the tile strikes it 0 or 1 times, so the tile
        // kernel's one division per (prime, tile) is nearly all waste for it.
        // Those go to sieve_mark_huge, which walks the whole segment by addition
        // and never divides.  See tf_kernel.cl.h.
        //
        // Both constants measured on an RTX 3070, p=86000009, 2^66..2^67, as
        // sieve_mark_large + sieve_mark_huge device time at sieve_primes=1037053
        // (`--profile`).  The baseline -- a 32768-bit tile and no huge tier at
        // all -- was 8.06 s.
        //
        //   threshold:   65536    131072   262144   524288
        //   tile 32768    1.12      0.99     1.66      --
        //   tile 65536    1.08      0.87     1.04     2.01
        //   tile 131072    --       0.87     0.85     1.36
        //   tile 262144    --        --      1.03     1.21
        //
        // Two things the table says.  For a fixed threshold a larger tile is
        // always better, because it is the divisions of the primes that STAY in
        // sieve_mark_large that it removes -- until the tile grows enough to cut
        // the group count below the device's appetite (262144 bits leaves 64
        // groups for 46 CUs).  And the threshold wants to sit well above the
        // tile, not at it: a prime just above the tile still strikes the segment
        // hundreds of times, and paying a global atomic for each beats one LDS
        // division per tile.  At the shipped sieve_primes there is no prime above
        // 262144 at all, so the huge tier is empty and this costs nothing there
        // (0.44 s -> 0.34 s from the larger tile alone).
        const uint32_t TILE_WANT  = 4096;                  // words: 131072 bits, 16 KB
        const uint64_t HUGE_MIN   = 262144;
        // CL 1.2 guarantees only 32 KB of local memory, and a device may offer
        // less than this tile wants; fall back rather than fail to launch.
        uint32_t tile_words = TILE_WANT;
        while (tile_words > 256 && (uint64_t)tile_words * 4 > g.lds) tile_words >>= 1;
        g_tile_words = tile_words;

        huge_first = large_first;
        while (huge_first < large_last && sp[huge_first].s < HUGE_MIN) ++huge_first;
    ...
    auto pick_kernel = [&](U128 level_hi_excl, cl_kernel& K, size_t& WG, const char*& nm, uint32_t& vpt) {
        ...
        // Three 28-bit limbs reach 2^82 with the same fifteen products and the same
        // unnormalised columns as the 24-bit path, so the band that used to fall
        // back to three 32-bit limbs -- and lose 21% doing it -- no longer does.
        // This is the band GIMPS actually trial-factors.  It is placed ahead of the
// eager 72-bit kernel because it also wins the narrow 2^70..2^72 strip
// (7.81 s against 8.83 s) -- that kernel is 1-wide and reduces every
// squaring, so the only thing it still has is being reachable by
// arithmetic = 72 for comparison.
        else if (!u128_gt(level_hi_excl, B82) && g.kernel84Lx2 && (a == 0 || a == 84)) {
    ...
    for (size_t li = level_start; li < levels.size() && !abort_flag.load(); ++li) {
        ...
        // The fused kernel compacts the bitmap itself, so it exists only for the
        // device sieve and (so far) only for the 72-bit lazy x2 path.
        // Only the 72-bit lazy x2 path has a fused twin -- see tf_kernel.cl.h;
        // the 96-bit one was written, measured no faster, and dropped.
        cl_kernel Kgs = nullptr;
        ...
        for (size_t cb = (li == level_start ? class_start : 0);
            ...
            while (queue.pop(b)) {
                ...
                if (cfg.sieve_on_gpu) {
                    ...
                    auto enq = [&](cl_kernel K, size_t gx, size_t gy, size_t lx, int kp) {
                        size_t gs[2] = { ((gx + lx - 1) / lx) * lx, gy };
                        size_t ls[2] = { lx, 1 };
                        cl_event e = nullptr;
                        cl_int r = CL.EnqueueNDRangeKernel(g.queue, K, gy > 1 ? 2 : 1, nullptr, gs, ls,
                                                           0, nullptr, g_profile ? &e : nullptr);
                        if (g_profile && r == CL_SUCCESS) kp_record(kp, e);
                        return r;
                    };
                    ...
                    enq(g.k_offsets, np, 1, 64, KP_OFFSETS);
                    ...
                    if (huge_first > large_first) {
                        ...
                        CL.SetKernelArg(g.k_large, a++, sizeof(uint32_t), &huge_first);
                        ...
                        enq(g.k_large, groups * LWG, 1, LWG, KP_LARGE);
                    }
                    // After mark_large: that kernel writes whole tiles back with
                    // plain stores and would clobber these bits.  In-order queue.
                    if (large_last > huge_first) {
                        a = 0;
                        CL.SetKernelArg(g.k_huge, a++, sizeof(cl_mem), &bits_buf[slot]);
                        CL.SetKernelArg(g.k_huge, a++, sizeof(cl_mem), &s_buf);
                        CL.SetKernelArg(g.k_huge, a++, sizeof(cl_mem), &offs_buf[slot]);
                        CL.SetKernelArg(g.k_huge, a++, sizeof(uint32_t), &huge_first);
                        CL.SetKernelArg(g.k_huge, a++, sizeof(uint32_t), &large_last);
                        CL.SetKernelArg(g.k_huge, a++, sizeof(uint32_t), &len);
                        enq(g.k_huge, large_last - huge_first, 1, 256, KP_HUGE);
                    }

                    a = 0;
                    if (fused) goto sieve_done;   // the TF kernel compacts it
                    ...
                    // COMPACT_WG in the kernel: the scan array is sized at compile
                    // time, so the local size is not the driver's to choose.
                    st = enq(g.k_compact, nw, 1, 256, KP_COMPACT);
                ...
                int argi = 0;
                // Fused: the kernel reads the bitmap and the segment length in place
                // of an index list and a count, and compacts for itself.
                const uint32_t fused_len = (uint32_t)live[slot].scanned;
                cl_kernel KK = fused ? Kgs : K;
                if (fused) {
                    CL.SetKernelArg(KK, argi++,sizeof(cl_mem),   &bits_buf[slot]);
                    CL.SetKernelArg(KK, argi++,sizeof(uint32_t), &fused_len);
                } else {
                    CL.SetKernelArg(KK, argi++,sizeof(cl_mem),   &idx_buf[slot]);
                    CL.SetKernelArg(KK, argi++,sizeof(cl_mem),   &n_buf[slot]);
                }
                CL.SetKernelArg(KK, argi++,sizeof(uint64_t), &base_lo);
                CL.SetKernelArg(KK, argi++,sizeof(uint64_t), &base_hi);
                CL.SetKernelArg(KK, argi++,sizeof(uint64_t), &step);
                CL.SetKernelArg(KK, argi++,sizeof(uint64_t), &twop);
                CL.SetKernelArg(KK, argi++,sizeof(uint64_t), &pe);
                CL.SetKernelArg(KK, argi++,sizeof(int32_t),  &pbits);
                CL.SetKernelArg(KK, argi++,sizeof(cl_mem),   &cnt_buf);
                CL.SetKernelArg(KK, argi++,sizeof(cl_mem),   &res_buf);
                CL.SetKernelArg(KK, argi++,sizeof(uint32_t), &FOUND_CAP);
                if (fused) CL.SetKernelArg(KK, argi++,sizeof(cl_mem), &ptot_buf);
                ...
                if (fused) {
                    // One group per 256 words of bitmap; each loops over what it
                    // compacted, so the grid is the segment's size, not the
                    // survivors' -- and there is no count to read back for it.
                    local  = TFGS_WG;
                    size_t nwf = ((size_t)fused_len + 31) / 32;
                    global = ((nwf + local - 1) / local) * local;
                }
                ...
                st = CL.EnqueueNDRangeKernel(g.queue, KK, 1, nullptr, &global, &local,
                                             nwait, nwait ? wait : nullptr, &evt[slot]);
                if (st != CL_SUCCESS) { err = "clEnqueueNDRangeKernel failed (" + std::to_string(st) + ")"; request_abort(); break; }
                ++launches;
                // evt[slot] is the pipeline's own slot-reuse fence, so read its
                // timestamps here rather than consuming it.
                if (g_profile && CL.GetEventProfilingInfo) {
                    cl_ulong t0 = 0, t1 = 0;
                    CL.WaitForEvents(1, &evt[slot]);
                    if (CL.GetEventProfilingInfo(evt[slot], CL_PROFILING_COMMAND_START, sizeof(t0), &t0, nullptr) == CL_SUCCESS &&
                        CL.GetEventProfilingInfo(evt[slot], CL_PROFILING_COMMAND_END,   sizeof(t1), &t1, nullptr) == CL_SUCCESS &&
                        t1 >= t0) { g_kp_ns[KP_TF] += (double)(t1 - t0); g_kp_n[KP_TF] += 1; }
                }
...
static int run_main(int argc, char** argv)
{
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        else if (a == "--no-fuse")                g_fuse = false;
    ...
    if (g_profile) {
        double tot = 0;
        for (int i = 0; i < KP_N; ++i) tot += g_kp_ns[i];
        printf("  per-kernel device time (--profile; the run's WALL TIME IS NOT\n"
               "  MEANINGFUL here -- every launch was waited on to read its stamps)\n");
        printf("    %-18s %10s %10s %8s %9s\n", "kernel", "seconds", "launches", "share", "us/launch");
        for (int i = 0; i < KP_N; ++i) {
            if (!g_kp_n[i]) continue;
            double s = g_kp_ns[i] / 1e9;
            printf("    %-18s %10.3f %10llu %7.1f%% %9.1f\n", KP_NAME[i], s,
                   (unsigned long long)g_kp_n[i], tot > 0 ? 100.0 * g_kp_ns[i] / tot : 0.0,
                   g_kp_ns[i] / 1e3 / (double)g_kp_n[i]);
        }
        printf("    %-18s %10.3f\n", "total", tot / 1e9);
        printf("\n");
    }
